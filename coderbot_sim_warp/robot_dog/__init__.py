...
class RobotDogExample:
    def __init__(self, use_cuda_graph=False, headless=False, num_envs=1):
        articulation_builder = wp.sim.ModelBuilder()

        rot_x = wp.quat_from_axis_angle(wp.vec3(1, 0, 0), -math.pi * 0.5)
        rot_y = wp.quat_from_axis_angle(wp.vec3(0, 1, 0),  math.pi * 0.5)
        xform = wp.transform(wp.vec3(0, 0.65, 0), rot_y * rot_x)
        ...
        for i in range(num_envs):
            ...
            builder.joint_act[-12:] = builder.joint_q[-12:]

            builder.joint_axis_mode = [
                wp.sim.JOINT_MODE_TARGET_POSITION
            ] * len(builder.joint_axis_mode)
